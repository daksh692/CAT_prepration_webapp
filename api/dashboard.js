/**
 * Dashboard API - Single Entry Point
 * Handles /api/dashboard/* via vercel.json rewrites
 */
...
async function handler(req, res) {
    ...
    if (pathParts.length === 0) return errorResponse(res, 400, 'Invalid dashboard route');
    ...
    try {
            ...
            if (req.method === 'PUT') {
                ...
                if (existing.length === 0) {
                    ...
                    if (daily_goal_questions !== undefined) { 
                       // Check if col exists, ignoring for now as per previous logic
                    }
        ...
        if (endpoint === 'session') {
            ...
            // Streak update logic (simplified copy)
            const rows = await executeQuery('SELECT * FROM streaks WHERE user_id = ?', [userId]);
            let streakData = {};
            if (rows.length === 0) {
